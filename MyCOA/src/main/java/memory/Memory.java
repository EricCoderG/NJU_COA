...
public class Memory {
    ...
    public void real_load(String pAddr, int len) {
        // TODO
        this.write(pAddr, len, this.disk.read(pAddr, len));
    }
    ...
    public void seg_load(int segIndex) {
        // TODO
        String address = String.join("", Collections.nCopies(32, "0"));
        // 从磁盘中加载数据
        // 地址就是基址, 这种情况是 32 个零
        if (!PAGE) { //!PAGE时由seg_load加载
            // 界限为20位
            int len = 1 << 20;
            this.write(address, len, disk.read(address, len));
        }
        //更新GDT
        SegDescriptor temp = getSegDescriptor(segIndex);
        temp.base = address.toCharArray();
        temp.limit = String.join("", Collections.nCopies(20, "1")).toCharArray();
        temp.validBit = true;
        // 段页和分段模式的粒度不一样
        temp.granularity = PAGE;
    }
    ...
    public void page_load(int vPageNo) {
        // TODO
        // 一页的大小为 4KB = 2^12 即 12 位, 即 PAGE_SIZE_B
        // 因此地址前20位为虚拟页号
        // 从磁盘中加载一页到内存中
        int len = PAGE_SIZE_B;
        String vPageNum = Transformer.intToBinary(String.valueOf(vPageNo)).substring(12);
        String VA = vPageNum + "000000000000";
        byte[] data = disk.read(VA, len);
        // 在主存中找到空闲页框, 并将主存的实际物理页框号记录在页表中, 与虚拟页号形成对应
        // 20位页框号
        String pageFrame = "";
